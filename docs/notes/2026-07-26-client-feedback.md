...
- **Why some posts show pairs and others singles:** batch-added
  ...
  rows. (Superseded by design change D1 below.)
...
- **F6 — One photo per row as the automatic arrangement** (design
...
- **B7 — Font unification: About and Inquire bodies must match Post
...
- **B8 — Publishes reportedly not reaching the live site (~2 hours).**
...
- **D3 decision needed (Sammy) — "fonts editable in Sanity":** Sammy
  ...
  design-consistency foot-gun for a one-editor site, and B7 removes
...
- **D1 (accepted):** automatic photo placement = one photo per row,
  full article width. Deliberate 2-up/3-up remains available via
  manual Image row.
- **D2 (accepted, revisit if pain persists):** stay on tags for media
...
1. **T1 — Diagnose & fix B8 (stale live site) — FIRST.**
...
4. **T4 — cheat-sheet update** for the client message: shift-click
